...
#define INITIAL_DATA_SIZE 10
#define DECIMAL_BASE 10
#define INVALID_POINTER_ERROR "ERROR: The pointer to LinkedList/node is an \
invalid pointer\n"
#define ALLOCATION_ERROR "ERROR: memory allocation failed\n"
#define INVALID_FILE_ERROR "ERROR: The given file is not a valid file\n"
#define INVALID_POINTER "ERROR: The pointer to 'num_elements_in_returned_\
array' is an invalid pointer \n"
 ...
 *    2. You may assume that all nodes and their data arrays are
 *    allocated dynamically.
 *    3. You may assume that there are no two nodes that their
 *    data fields point to the
 *       same array.
 */
...
typedef struct Node {
  struct Node *next; // this member should allow access to next node
  // in the list. (think - pointer or instance?)
  struct Node *prev; // this member should allow access to next node
  // in the list. (think - pointer or instance?)
  int *data; // this member should be an array of ints.
  // (think - signed or unsigned?)
  size_t len; //this member should be the number of elements in the data array.
  // (think - signed or unsigned?)
} Node;

typedef struct LinkedList {
  Node *head; // this member should allow access to the first node in
  // the linked list. (think - pointer or instance?)
  Node *tail; // this member should allow access to the node at the
  // end of the linked list.
  // (think - pointer or instance?)
} LinkedList;
 ...
 * Assumptions:
 *    You cannot assume the pointers are valid.
 *    You cannot assume the node is in the list.
 *    In this case don't free its resources.
 *    To be sure - you are supposed to compare the given node to
 *    the nodes in the list using == operator.
 *
 * In case of errors:
 ...
 * (for nodes with data != NULL) when the function finishes execution. To be
 * clear - at the start of the function,
 ...
 * calculate the grade average of every node and save it into an array, which
 * you will return from the function.
 */
double *
get_averages (LinkedList *list, size_t *num_elements_in_returned_array);

/**
 * opens a file from a given filename and parses
 * it's contents into a LinkedList.
 *
 * Assumptions:
